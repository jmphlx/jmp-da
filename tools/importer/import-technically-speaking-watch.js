...
const createResources = (document) => {
  const cells = [
    ['columns (text-link-caret-left)'],
  ]
  const lhText = document.querySelector('div.parsys_column.cq-colctrl-lt2 div.parsys_column.cq-colctrl-lt2-c0 div.text.parbase.section');
  if (lhText) {
    console.log("LOOK HERE");
    console.log(lhText);
    const link = lhText.querySelector("a");
    console.log(link.href);
    const href = link.href;
    const path = href.slice(21);
    console.log(path);
    link.href = "https://www.jmp.com" + path;
    cells.push([lhText]);
  if (cells.length > 1) return WebImporter.DOMUtils.createTable(cells, document);
  };
...
export default {
  transformDOM: ({ document }) => {
    ...
    const resource = createResources(document);
    if (resource) section.append(resource);
