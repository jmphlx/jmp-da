...
const embedHubspot = (fRegion, fPortalId, fFormId, sfdcCampaignId = null) => {
  ...
  if (sfdcCampaignId) { //check if form has a salesforce campaign id
  ...
  } else { //function to call if it doesn't
    scriptHubspot.addEventListener('load', () => {
      hbspt.forms.create({
        region: fRegion,
        portalId: fPortalId,
...
export default function decorate(block) {
  const [region, portalId, formID, sfdcCampaignId] = [...block.children].map((c) => c.firstElementChild); //mapping variables
  ...
  if (sfdcCampaignId){  const sfdcCampaignIdVal = sfdcCampaignId.textContent;
  }

  block.textContent = '';

  const observer = new IntersectionObserver((entries) => { //calling embed function
